...
class White {
    ...
    private int value = 0;
    private String color;
    private List<White> lists = new ArrayList<>();
    private String pathStr = "";
    boolean onlyMax = false;
    public boolean isFinished = false;
    private boolean campIn = false;
    ...
    public White(char[][] map, String color) {
        this.color = color;
        this.map = map;
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < 16; j++) {
    ...
    public White(char[][] map, String color, String pathStr, int newX, int newY) {
        this.color = color;
        this.map = map;
        this.pathStr = pathStr;
        this.newX = newX;
        ...
        for (Point point : white) {
            ...
            if (!blackCamp.contains(point) && !whiteCamp.contains(point)) {
                ...
                if ((x - 1) >= 2 && (y - 1) >= 2 && (x + 1) <= 13 && (y + 1) <= 13) {
                    ...
                    if (map[y][x - 1] == '.' && map[y - 1][x - 1] == '.' && map[y - 1][x] == '.') {
                        val -= 199;
                        if (map[y - 1][x + 1] == '.' && map[y + 1][x - 1] == '.' && map[y][x + 1] == '.' && map[y + 1][x + 1] == '.' && map[y + 1][x] == '.') {
                            val -= 99;
                        }
                    }
                } else {
            ...
            } else if (whiteCamp.contains(point)) {
                int a = x + y - 14;
                val = val + (f1 * a * a);
                if (map[y - 1][x - 1] != '.' && map[y - 2][x - 2] == '.') {
                    val = val + (8 * a * a);
    ...
    public List<White> nextSteps(boolean isDeadBoard) {
        if (this.color.equals("BLACK")) {
            blackNext(isDeadBoard);
        } else {
            whiteNext(isDeadBoard);
        ...
        if (this.campIn && this.lists.size() > 0) {
            List<White> list1 = new ArrayList<>();
            List<White> list2 = new ArrayList<>();
            Point p;
            for (White list : lists) {
                p = new Point(list.newX, list.newY);
                if (blackCamp.contains(p) || whiteCamp.contains(p)) {
                    list2.add(list);
                } else {
    ...
    public void walk(int x, int y, List<String> path) {
        if ((y - 1) >= 0 && (x - 1 >= 0) && map[y - 1][x - 1] == '.') {
            path.add((x - 1) + "," + (y - 1));
            White temp = newState(y, x, y - 1, x - 1, path, false);
            if (temp != null) {
                this.lists.add(temp);
            }
            path.remove(path.size() - 1);
        ...
        if ((y - 1) >= 0 && map[y - 1][x] == '.') {
            path.add((x) + "," + (y - 1));
            White temp = newState(y, x, y - 1, x, path, false);
            if (temp != null) {
                this.lists.add(temp);
            }
            path.remove(path.size() - 1);
        ...
        if ((x + 1 < 16) && map[y][x + 1] == '.') {
            path.add((x + 1) + "," + (y));
            White temp = newState(y, x, y, x + 1, path, false);
            if (temp != null) {
                this.lists.add(temp);
            }
            path.remove(path.size() - 1);
    ...
    private void jump(int x, int y, List<Point> isVisited, int oriX, int oriY, List<String> path) {
        ...
        if ((y - 2) >= 0 && (x - 2) >= 0 && map[y - 2][x - 2] == '.' && map[y - 1][x - 1] != '.' && !isVisited.contains(p)) {
            path.add(p.x + "," + p.y);
            White temp = newState(oriY, oriX, y - 2, x - 2, path, true);
            if (temp != null) {
                this.lists.add(temp);
            }
            isVisited.add(p);
        ...
        if ((y - 2) >= 0 && map[y - 2][x] == '.' && map[y - 1][x] != '.' && !isVisited.contains(p)) {
            path.add(p.x + "," + p.y);
            White temp = newState(oriY, oriX, y - 2, x, path, true);
            if (temp != null) {
                this.lists.add(temp);
            }
            isVisited.add(p);
        ...
        if ((x + 2) < 16 && map[y][x + 2] == '.' && map[y][x + 1] != '.' && !isVisited.contains(p)) {
            path.add(p.x + "," + p.y);
            White temp = newState(oriY, oriX, y, x + 2, path, true);
            if (temp != null) {
                this.lists.add(temp);
            }
            isVisited.add(p);
    ...
    public White newState(int y, int x, int newY, int newX, List<String> path, boolean isJump) {
        Point ori = new Point(x, y);
        Point newL = new Point(newX, newY);
        ...
        String str = sb.toString();
        if (!onlyMax) {
            return new White(clone, this.color.equals("BLACK") ? "WHITE" : "BLACK", str, newX, newY);
        } else {
            return new White(clone, this.color, str, newX, newY);
        }
    }
